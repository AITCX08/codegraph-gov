...
def build_index(embedder=None, limit: int | None = None, batch: int = 256,
    ...
    dim = int(vecs_parts[0].shape[1]) if vecs_parts else int(getattr(embedder, "dimension", 384))
    vecs = np.vstack(vecs_parts) if vecs_parts else np.zeros((0, dim), dtype=np.float32)
